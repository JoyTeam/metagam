    ...
    if (res.redirect) {
        ...
        try {
            if (window.opera && opera.toString() == "[object Opera]") {
                if (opera.version().match(/^(9\.|10\.)/)) {
                    if (!res.advice) {
                        res.advice = [];
                    }
                    res.advice.splice(0, 0, {
                        title: gt.gettext('Opera Warning'),
                        content: gt.gettext('You are using old version of Opera browser. It has known bugs that may prevent form uploads from working. Please upgrade to the recent Opera version or use Firefox or Chrome.'),
                        cls: 'admin-advice-body-warning'
                    });
                }
            }
        } catch (e) {
        ...
        if (res.advice && res.advice.length) {
            res.advice[res.advice.length - 1].lst = true;
            advicecontent.add({
