...
class Cluster(Module):
    ...
    def upload(self, subdir, ext, content_type, data, filename=None):
        ...
        id = uuid4().hex
        if filename is None:
            url = str("/data/%s-%s/%s%s/%s/%s.%s" % (subdir, tag[0], tag[0], tag[1], tag, id, ext))
        else:
            url = str("/data/%s-%s/%s%s/%s/%s-%s" % (subdir, tag[0], tag[0], tag[1], tag, id, filename))
        uri = str("//" + host + url)
        cnn = HTTPConnection()
        cnn.connect((str(host), 80))
        try:
    ...
    def static_upload_zip(self, subdir, zip, upload_list):
        ...
        id = uuid4().hex
        tag = self.app().tag
        url = str("/data/%s-%s/%s%s/%s/%s" % (subdir, tag[0], tag[0], tag[1], tag, id))
        uri = str("//" + host + url)
