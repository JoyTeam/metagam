...
class ReqAuction(ConstructorModule):
    ...
    def cert_check(self):
        req = self.req()
        mcid = "wm-certified-%s" % req.user()
        if self.app().mc.get(mcid):
            return
        ...
        for wmid in wmids:
            ...
            if cert >= 120:
                self.app().mc.set(mcid, 1, 86400)
                return
            if cert > lvl:
                lvl = 0
        self.error(self._("Certificate required"), self._("To use request auction you must have WMID with the starter certificate. Your current certificate level is not enough"))
