...
class Auth(Module):
    ...
    def admin_characters_form(self):
        ...
        if req.args:
            ...
            if req.ok():
                ...
                self.call("admin.redirect", "characters/form")
            fields = [
                {"name": "name", "label": self._("Short description for administrators"), "value": name, "condition": "![std]", "inline": True },
            ...
            self.call("admin.form", fields=fields)
        self.htmlescape_character_form(character_form)
    ...
    def htmlescape_character_form(self, lst):
        for fld in lst:
            fld["name"] = htmlescape(fld.get("name"))
            fld["description"] = htmlescape(fld.get("description"))
            fld["prompt"] = htmlescape(fld.get("prompt"))

    def character_form(self):
        fields = self.conf("auth.char_form", default=[])
        if not len(fields):
            fields.append({"std": 1, "code": "name", "name": self._("Character name"), "order": 10.0})
            fields.append({"std": 2, "code": "sex", "name": self._("Character sex"), "order": 20.0})
